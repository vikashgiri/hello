...
public class TypeOfContractDialogPostJobFragment extends DialogFragment {
    ...
    private void init() {
        ...
        String[] mTestArray=  new String[getJobpostDropDown.getContractTypes()
                .size()];;
        for(int i=0;i< getJobpostDropDown.getContractTypes()
                .size();i++)
        {
            mTestArray[i]=getJobpostDropDown.getContractTypes().get(i).getContract_title();
                    ...
                    for (int i = 0; i < checked.size(); i++) {
                        ...
                        if (checked.valueAt(i)) {
                            ...
                            if (str == null) {
                                ...
                                valueId=getJobpostDropDown.getContractTypes().get(position)
                                        .getContract_id();
                                ...
                                valueId=valueId+ ","+getJobpostDropDown.getContractTypes().get(position)
                                        .getContract_id();
                            }
                        }
                    }
        ...
        if (!bundle.containsKey(Keys.TYPE)) {
...

            {
                ...
                intent.putExtra(Keys.TYPE_OF_CONTRACT_ID,getJobpostDropDown
                        .getContractTypes().get(position).getContract_id());
                getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, intent);
                getDialog().dismiss();
            });
        }
