...
public class DiplomaDialogJobPostFragment extends DialogFragment
{
    JsonArray array;
    ...
    private void init() {
            ...
            public void onClick(View v)
            {
                ...
                if (checked.size() > 0) {
                    ...
                    for (int i = 0; i < checked.size(); i++) {
                        ...
                        if (checked.valueAt(i)) {
                            ...
                            if (str == null) {
                                ...
                                valueId=getJobpostDropDown.getEducationLevels().get(position)
                                        .getEducation_id();
                            } else {
                                str = str + "#" + position;
                                value=value + ","+arrayAdapter.getItem(position);
                                valueId=valueId+ ","+getJobpostDropDown.getEducationLevels().get(position)
                                        .getEducation_id();
                            }
                        }
                    }
                    //if (array.size() > 0) {
                        ...
                        intent.putExtra(Keys.DIPLOMA, value);
                    intent.putExtra(Keys.DIPLOMA_ID, valueId);
       ...
       // String[] mTestArray = getResources().getStringArray(R.array.education_level);
        ...
        String[] mTestArray=  new String[getJobpostDropDown.getEducationLevels()
                .size()];
        for(int i=0;i< getJobpostDropDown.getEducationLevels().size();i++)
        {
            mTestArray[i]=getJobpostDropDown.getEducationLevels().get(i).getEducation_title();

            // String[] mTestArray = getResources().getStringArray(R.array.array_type_of_contract);
        }
        arrayAdapter = new ArrayAdapter<String>(getActivity(), R.layout.list_item_single_choice, mTestArray) {
            @NonNull
            @Override
        ...
        listView.setOnItemClickListener((parent, views, position, id) -> {
            ...
            intent.putExtra(Keys.DIPLOMA_ID, getJobpostDropDown.getEducationLevels()
            .get(position).getEducation_id());
            getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, intent);
            getDialog().dismiss();
        });
        }
