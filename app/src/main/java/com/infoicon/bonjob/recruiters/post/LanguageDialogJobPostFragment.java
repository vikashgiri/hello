...
public class LanguageDialogJobPostFragment extends DialogFragment {
    ...
    private void init() {
      ...
      //  String[] mTestArray = getResources().getStringArray(R.array.array_language);
        String[] mTestArray=  new String[getJobpostDropDown.getJobLanguages()
                .size()];
        for(int i=0;i< getJobpostDropDown.getJobLanguages().size();i++)
        {
            mTestArray[i]=getJobpostDropDown.getJobLanguages().get(i)
            ...
            public void onClick(View v) {
                ...
                if (checked.size() > 0) {
                    ...
                    for (int i = 0; i < checked.size(); i++) {
                        ...
                        if (checked.valueAt(i)) {
                            ...
                            if (str == null) {
                                ...
                                valueId=getJobpostDropDown.getJobLanguages().get(position)
                                        .getJob_language_title();
                            } else {
                                str = str + "#" + position;
                                value=value + ","+arrayAdapter.getItem(position);
                                valueId=valueId+ ","+getJobpostDropDown.getJobLanguages().get(position)
                                        .getJob_language_id();
                            }
                        }
                    }
                  //  if (array.size() > 0) {
                    ...
                    intent.putExtra(Keys.LANGUAGEID, valueId);
        ...
        if (!bundle.containsKey(Keys.TYPE)) {
            ...
            listView.setOnItemClickListener((parent, views, position, id) -> {
                ...
                intent.putExtra(Keys.LANGUAGEID, getJobpostDropDown.getJobLanguages()
                .get(position).getJob_language_id());
                getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, intent);
                getDialog().dismiss();
            });
        }
