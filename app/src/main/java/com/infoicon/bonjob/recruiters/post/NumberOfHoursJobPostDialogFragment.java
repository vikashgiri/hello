...
public class NumberOfHoursJobPostDialogFragment extends DialogFragment {
    ...
    private void init() {
        ...
        tvTitle.setText(getResources().getString(R.string.num_of_hours));
       ...
       // String[] mTestArray = getResources().getStringArray(R.array.array_num_hour);

        String[] mTestArray=  new String[getJobpostDropDown.getNumberOfHours()
                .size()];
        for(int i=0;i< getJobpostDropDown.getNumberOfHours().size();i++)
        {
            mTestArray[i]=getJobpostDropDown.getNumberOfHours().get(i).getHours_title();
            ...
            public void onClick(View v) {
                ...
                if (checked.size() > 0) {
                    ...
                    for (int i = 0; i < checked.size(); i++) {
                        ...
                        if (checked.valueAt(i)) {
                            ...
                            if (str == null) {
                                ...
                                valueId=getJobpostDropDown.getNumberOfHours().get(position)
                                        .getHours_id();

                            } else {
                                str = str + "#" + position;
                                value=value + ","+arrayAdapter.getItem(position);
                                valueId=valueId+ ","+getJobpostDropDown.getNumberOfHours().get(position)
                                        .getHours_id();
                            }
                        }
                    }
                   ...
                   // if (array.size() > 0) {
                        ...
                        intent.putExtra(Keys.NUM_HOUR, value);
                    intent.putExtra(Keys.NUM_HOUR_ID, valueId);
        ...
        if (!bundle.containsKey(Keys.TYPE)) {
            ...
            listView.setOnItemClickListener((parent, views, position, id) -> {
                ...
                intent.putExtra(Keys.NUM_HOUR, mTestArray[position]);
                intent.putExtra(Keys.NUM_HOUR_ID, getJobpostDropDown.getNumberOfHours().get(position)
                .getHours_id());
                getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, intent);
                getDialog().dismiss();
            });
