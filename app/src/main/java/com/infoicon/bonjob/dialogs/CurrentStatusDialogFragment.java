...
public class CurrentStatusDialogFragment extends DialogFragment {
    ...
    private void init() {
        ...
        tvTitle.setText(getResources().getString(R.string.current_status));
        ...
        String[] mTestArray=  new String[jobSearchDropDownResponce.getCurrentStatuses()
                .size()];
        for(int i=0;i< jobSearchDropDownResponce.getCurrentStatuses().size();i++)
        {
            mTestArray[i]=jobSearchDropDownResponce.getCurrentStatuses().get(i)
                    .getSeeker_current_status_title();

            // String[] mTestArray = getResources().getStringArray(R.array.array_type_of_contract);
        }
     //   String[] mTestArray = getResources().getStringArray(R.array.array_status_actual);
        ArrayAdapter<String> arrayAdapter = new
                ArrayAdapter<String>(getActivity(), R.layout.list_item_single_choice, mTestArray) {
            @NonNull
            @Override
            ...
            public void onClick(View v) {
                ...
                if (checked.size() > 0) {
                    ...
                    for (int i = 0; i < checked.size(); i++) {
                        ...
                        if (checked.valueAt(i)) {
                            ...
                            if (str == null) {
                                ...
                                valueId=jobSearchDropDownResponce.getCurrentStatuses().get(position)
                                        .getSeeker_current_status_id();

                            } else {
                                str = str + "#" + position;
                                value=value + ","+arrayAdapter.getItem(position);
                                valueId=valueId+ ","+jobSearchDropDownResponce.
                                        getCurrentStatuses().get(position)
                                        .getSeeker_current_status_id();
                            }
                        }
                    }
                   // if (array.size() > 0) {
                        ...
                        intent.putExtra(Keys.CURRENT_STATUS,value);
                        intent.putExtra(Keys.CURRENT_STATUS_ID,valueId);
        ...
        if (!bundle.containsKey(Keys.TYPE)) {
            ...
            listView.setOnItemClickListener((parent, views, position, id) -> {
                ...
                intent.putExtra(Keys.CURRENT_STATUS, mTestArray[position]);
                getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, intent);
                getDialog().dismiss();
            });
        }
