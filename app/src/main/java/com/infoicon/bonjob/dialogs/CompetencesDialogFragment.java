...
public class CompetencesDialogFragment extends DialogFragment {
    ...
    private void init()
    {
        ...
        tvTitle.setText(getResources().getString(R.string.compétences));
        ...
        String[] mTestArray=  new String[jobSearchDropDownResponce.getSkills()
                .size()];
        for(int i=0;i< jobSearchDropDownResponce.getSkills().size();i++)
        {
            mTestArray[i]=jobSearchDropDownResponce.getSkills().get(i).getSkill_title();
       ...
       // String[] mTestArray = getResources().getStringArray(R.array.array_active_area);
            ...
            public void onClick(View v) {
                ...
                if (checked.size() > 0) {
                    ...
                    for (int i = 0; i < checked.size(); i++) {
                        ...
                        if (checked.valueAt(i)) {
                            ...
                            if (str == null) {
                                ...
                                valueId=jobSearchDropDownResponce.getSkills().get(position)
                                        .getSkill_id();
                            } else {
                                str = str + "#" + position;
                                value=value + ","+arrayAdapter.getItem(position);
                                valueId=valueId+ ","+jobSearchDropDownResponce.getSkills().get(position)
                                        .getSkill_id();
                            }
                        }
                    }
                   // if (array.size() > 0) {
                        ...
                        intent.putExtra(Keys.COMPETENCES,value);
                        intent.putExtra(Keys.COMPETENCES_ID,valueId);
        ...
        if (!bundle.containsKey(Keys.TYPE)) {
            ...
            listView.setOnItemClickListener((parent, views, position, id) -> {
                ...
                intent.putExtra(Keys.COMPETENCES, mTestArray[position]);
                getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, intent);
                getDialog().dismiss();
            });
        }
